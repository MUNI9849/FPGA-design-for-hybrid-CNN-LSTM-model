...
//inline T16 hard_tanh(T16 x) {
inline T16 tanh_T16(T16 x) {
    if (x <= -1) {
        return -1;
    } else if (x >= 1) {
        return 1;
...
void lstm_acc(T16 *ly1_inpw, T16 *ly1_rw, T16 *ly1_b, T16 *ly2_inpw, T16 *ly2_rw, T16 *ly2_b, T16 *dw, T16 *inputx, T16 *outputx);
void cnn_acc(T16 *conv1_w, T16 *conv1_bias,  T16 *conv2_w, T16 *conv2_bias, T16 *conv3_w, T16 *conv3_bias, T16 *fc1u_w, T16 *fc1v_w, T16 *fc1_bias, T16 *fc2u_w, T16 *fc2v_w, T16 *fc2_bias, T16 *fc3_w, T16 *fc3_bias, T16 *input_data, T16 *output_data);
...
T16 input_seq[3];
T16 input;
T16 lstm1_input_weights[INPUT_SIZE][4 * HIDDEN_SIZE1];
T16 lstm1_recurrent_weights[HIDDEN_SIZE1][4 * HIDDEN_SIZE1];
#pragma HLS ARRAY_PARTITION variable=lstm1_recurrent_weights cyclic factor=8 dim=1
T16 lstm1_bias[4 * HIDDEN_SIZE1];
T16 lstm2_input_weights[HIDDEN_SIZE1][4 * HIDDEN_SIZE2];
#pragma HLS ARRAY_PARTITION variable=lstm2_input_weights cyclic factor=4 dim=1
T16 lstm2_recurrent_weights[HIDDEN_SIZE2][4 * HIDDEN_SIZE2];
#pragma HLS ARRAY_PARTITION variable=lstm2_recurrent_weights cyclic factor=4 dim=1
T16 lstm2_bias[4 * HIDDEN_SIZE2];
T16 dense_weights[HIDDEN_SIZE2][OUTPUT_SIZE];
T16 dense_bias[OUTPUT_SIZE] = {0.04070737};
...
dense_weights[x][0] =  *dw++;
    ...
    static T16 c1[HIDDEN_SIZE1] = {0.0};
    static T16 h1[HIDDEN_SIZE1] = {0.0};
    static T16 c2[HIDDEN_SIZE2] = {0.0};
    static T16 h2[HIDDEN_SIZE2] = {0.0};
	...
	T16 input_gate_tmpx[HIDDEN_SIZE1];
	T16 forget_gate_tmpx[HIDDEN_SIZE1];
	T16 cell_state_tmpx[HIDDEN_SIZE1];
	T16 output_gate_tmpx[HIDDEN_SIZE1];
	...
	T16 input_gate_tmpy[HIDDEN_SIZE2];
	T16 forget_gate_tmpy[HIDDEN_SIZE2];
	T16 cell_state_tmpy[HIDDEN_SIZE2];
	T16 output_gate_tmpy[HIDDEN_SIZE2];
	...
	T16 a1input_gate_tmp;
	T16 a1forget_gate_tmp;
	T16 a1cell_state_tmp;
	T16 a1output_gate_tmp;  
	
	T16 b1input_gate_tmp;
	T16 b1forget_gate_tmp;
	T16 b1cell_state_tmp;
	T16 b1output_gate_tmp;  
	
	T16 c1input_gate_tmp;
	T16 c1forget_gate_tmp;
	T16 c1cell_state_tmp;
	T16 c1output_gate_tmp;  
	
	T16 d1input_gate_tmp;
	T16 d1forget_gate_tmp;
	T16 d1cell_state_tmp;
	T16 d1output_gate_tmp;  		
	
	T16 e1input_gate_tmp;
	T16 e1forget_gate_tmp;
	T16 e1cell_state_tmp;
	T16 e1output_gate_tmp;  
	
	T16 f1input_gate_tmp;
	T16 f1forget_gate_tmp;
	T16 f1cell_state_tmp;
	T16 f1output_gate_tmp;  
	
	T16 g1input_gate_tmp;
	T16 g1forget_gate_tmp;
	T16 g1cell_state_tmp;
	T16 g1output_gate_tmp;  
	
	T16 h1input_gate_tmp;
	T16 h1forget_gate_tmp;
	T16 h1cell_state_tmp;
	T16 h1output_gate_tmp;  					
	
	T16 i1input_gate_tmp;
	T16 i1forget_gate_tmp;
	T16 i1cell_state_tmp;
	T16 i1output_gate_tmp;  					
	
	T16 j1input_gate_tmp;
	T16 j1forget_gate_tmp;
	T16 j1cell_state_tmp;
	T16 j1output_gate_tmp;  		
	
	T16 k1input_gate_tmp;
	T16 k1forget_gate_tmp;
	T16 k1cell_state_tmp;
	T16 k1output_gate_tmp;  				
			
	T16 ly2xinput_gate_tmp;
	T16 ly2xforget_gate_tmp;
	T16 ly2xcell_state_tmp;
	T16 ly2xoutput_gate_tmp;   
	T16 ly2yinput_gate_tmp;
	T16 ly2yforget_gate_tmp;
	T16 ly2ycell_state_tmp;
	T16 ly2youtput_gate_tmp;   		 	
	
    	T16 input_gate[HIDDEN_SIZE1];
    	T16 forget_gate[HIDDEN_SIZE1];
    	T16 cell_state[HIDDEN_SIZE1];
    	T16 output_gate[HIDDEN_SIZE1];
    	
    	T16 input_gate2[HIDDEN_SIZE2];
    	T16 forget_gate2[HIDDEN_SIZE2];
    	T16 cell_state2[HIDDEN_SIZE2];
    	T16 output_gate2[HIDDEN_SIZE2];    	
    ...
    Loop_x10:for (int k = 0; k < 55; k++) 
	{
    	...
    	Loop_x11:for (int i = 0; i < HIDDEN_SIZE1; i++) 
		{
        	...
        	Loop_x12:for (int j = 0; j < INPUT_SIZE; j++) {
            	...
            	a1forget_gate_tmp += lstm1_input_weights[j][i + 8] * input + lstm1_bias[i + 8];;
            	a1cell_state_tmp += lstm1_input_weights[j][i + 16] * input + lstm1_bias[i + 16];;
            	a1output_gate_tmp += lstm1_input_weights[j][i + 24] * input + lstm1_bias[i + 24];;
        	}

        	Loop_x13:for (int j = 0; j < HIDDEN_SIZE1; j++) {
        		#pragma HLS unroll factor=8
            	input_gate_tmpx[j] = lstm1_recurrent_weights[j][i] * h1[j];
            	forget_gate_tmpx[j] = lstm1_recurrent_weights[j][i + 8] * h1[j];
            	cell_state_tmpx[j] = lstm1_recurrent_weights[j][i + 16] * h1[j];
            	output_gate_tmpx[j] = lstm1_recurrent_weights[j][i + 24] * h1[j];
        	}
			...
			k1output_gate_tmp	=	i1output_gate_tmp	+	j1output_gate_tmp;
        		...
        		input_gate[i] = sigmoid_T16(	k1input_gate_tmp);
        		forget_gate[i] = sigmoid_T16(	k1forget_gate_tmp);
        		cell_state[i] = tanh_T16(		k1cell_state_tmp);
        		output_gate[i] = sigmoid_T16(	k1output_gate_tmp);
    	}
    	Loop_x14:for(int i=0;i<64;i++)
    	{
    			c1[i] = forget_gate[i] * c1[i] + input_gate[i] * cell_state[i];
        		h1[i] = output_gate[i] * tanh_T16(c1[i]);
		}
    	...
    	Loop_x15:for (int i = 0; i < HIDDEN_SIZE2; i++) 
		{
        	...
        	Loop_x16:for (int j = 0; j < HIDDEN_SIZE1; j++) {
            	...
            	forget_gate_tmpx[j] = lstm2_input_weights[j][i + 8] * h1[j];
            	cell_state_tmpx[j] = lstm2_input_weights[j][i + 16] * h1[j];
            	output_gate_tmpx[j] = lstm2_input_weights[j][i + 24] * h1[j];
        	}

        	Loop_x17:for (int j = 0; j < HIDDEN_SIZE2; j++) {
        		#pragma HLS unroll factor=4
            	input_gate_tmpy[j] = lstm2_recurrent_weights[j][i] * h2[j];
            	forget_gate_tmpy[j] = lstm2_recurrent_weights[j][i + 8] * h2[j];
            	cell_state_tmpy[j] = lstm2_recurrent_weights[j][i + 16] * h2[j];
            	output_gate_tmpy[j] = lstm2_recurrent_weights[j][i + 24] * h2[j];
        	}

        	ly2xinput_gate_tmp += lstm2_bias[i];
        	ly2xforget_gate_tmp += lstm2_bias[i + 8];
        	ly2xcell_state_tmp += lstm2_bias[i + 16];
        	ly2xoutput_gate_tmp += lstm2_bias[i + 24];
        	...
        	input_gate2[i] = sigmoid_T16(ly2xinput_gate_tmp + 	ly2yinput_gate_tmp);
        	forget_gate2[i] = sigmoid_T16(ly2xforget_gate_tmp + 	ly2yforget_gate_tmp);	
        	cell_state2[i] = tanh_T16(ly2xcell_state_tmp + 		ly2ycell_state_tmp);
        	output_gate2[i] = sigmoid_T16(ly2xoutput_gate_tmp + 	ly2youtput_gate_tmp);
    	}
    
    	Loop_x18:for(int i=0;i<32;i++)
    	{
    		c2[i] = forget_gate2[i] * c2[i] + input_gate2[i] * cell_state2[i];
        	h2[i] = output_gate2[i] * tanh_T16(c2[i]);
		}	
	}
...
void cnn_acc(T16 *conv1_w, T16 *conv1_bias,  T16 *conv2_w, T16 *conv2_bias, T16 *conv3_w, T16 *conv3_bias, T16 *fc1u_w, T16 *fc1v_w, T16 *fc1_bias, T16 *fc2u_w, T16 *fc2v_w, T16 *fc2_bias, T16 *fc3_w, T16 *fc3_bias, T16 *input_data, T16 *output_data)
{
     ...
     ********* REGISTER DEFINITION**************
     *********************************/
    T16 input_data_buf[input_channel][input_R][input_C];
//#pragma HLS ARRAY_PARTITION variable=input_data_buf complete dim=1
    T16 conv1_out[CONV1_CHout][CONV1_R][CONV1_C];
#pragma HLS ARRAY_PARTITION variable=conv1_out cyclic factor=8 dim=1
    T16 pooling1_out[POOLING1_CHout][POOLING1_R][POOLING1_C+2];
//#pragma HLS ARRAY_PARTITION variable=pooling1_out cyclic factor=8 dim=1
    T16 conv2_out[CONV2_CHout][CONV2_R][CONV2_C];
#pragma HLS ARRAY_PARTITION variable=conv2_out cyclic factor=8 dim=1
    T16 pooling2_out[POOLING2_CHout][POOLING2_R][POOLING2_C+2];
    T16 conv3_out[CONV3_CHout][CONV3_R][CONV3_C];
    #pragma HLS ARRAY_PARTITION variable=conv3_out cyclic factor=8 dim=1
    T16 pooling3_out_buf[POOLING2_CHout][POOLING2_R][POOLING2_C];
    T16 pooling3_out[POOLING3_CHout*POOLING3_C*POOLING3_R];
    T16 fc1_out[FC1_OUTPUT_NUM1];
    T16 fc2_out[FC2_OUTPUT_NUM1];
    T16 output_data_buf[output_length];
    T16 fc1_weights_buf[FC1_INPUT_NUM1];
    T16 fc1_bias_buf[FC1_OUTPUT_NUM1];
    T16 conv1_W_buf[CONV1_CHout][CONV1_CHin][1][CONV_K];
...
#pragma HLS ARRAY_PARTITION variable=conv1_W_buf cyclic factor=8 dim=1
    T16 conv1_bias_buf[CONV1_CHout];
    T16 conv2_W_buf[CONV2_CHout][CONV2_CHin];
#pragma HLS ARRAY_PARTITION variable=conv2_W_buf cyclic factor=8 dim=1
T16 conv3_W_buf[CONV3_CHout][CONV3_CHin];
#pragma HLS ARRAY_PARTITION variable=conv3_W_buf cyclic factor=8 dim=1
    T16 conv2_bias_buf[CONV2_CHout];
    T16 conv3_bias_buf[CONV3_CHout];
    T16 fc2_weights_buf[FC2_INPUT_NUM1];
    T16 fc2_bias_buf[FC2_OUTPUT_NUM1];
    T16 fc3_weights_buf[FC3_INPUT_NUM1];
    T16 fc3_bias_buf[FC3_OUTPUT_NUM1];
    T16 res = 0.0;
    T16 fc1_outu[6];
    T16 fc2_outu[6];
     ...
     *********************************/
		...
		{			
				Loop_cv8:for (int m = 0; m < CONV1_C; m++)
				{
					...
					conv1_out[i][0][m] = (conv1_out[i][0][m] > res) ? conv1_out[i][0][m] :  res;
     ...
     *********************************/
        ...
        Loop2_cv9:for (int i = 0; i < CONV2_CHout; i++)
		{
				Loop2_cv10:for (int m = 0; m < CONV2_C; m++)
				{
					...
					conv2_out[i][0][m] = (conv2_out[i][0][m] >  res) ? conv2_out[i][0][m] :  res;
				}
		}
     ...
     *********************************/
        ...
        Loop3_cv9:for (int i = 0; i < CONV3_CHout; i++)
		{
				Loop3_cv10:for (int m = 0; m < CONV3_C; m++)
				{
					...
					conv3_out[i][0][m] = (conv3_out[i][0][m] >  res) ? conv3_out[i][0][m] :  res;
				}
		}
...
//  flattening layer
     ...
     *********************************/
        ...
        Loop1_fc9:for (int i = 0; i < FC1_OUTPUT_NUM1; i++)
        {
            ...
            fc1_out[i] = (fc1_out[i] >  0.0) ? fc1_out[i] :  res;
        }     
    /****************************************
     ...
     *********************************/     
        ...
        Loop2_fc5:for (int i = 0; i < FC2_OUTPUT_NUM1; i++)
        {
            ...
            fc2_out[i] = (fc2_out[i] >  0) ? fc2_out[i] :  res;
           // printf("%f \n", fc2_out[i]);
        }     
